...
DOMAIN_NAME = "bank"
INPUT_CSV_FILENAME = "bank_fraud_dataset_rebalanced_v3.csv"
...
def feature_engineering_and_vectorize_task():
    ...
    with mlflow.start_run(run_name=f"{DOMAIN_NAME}_feature_engineering_run") as run:
        ...
        # --- FIX APPLIED HERE ---
        # The 'recipient_id' column was removed from the list because it does not exist in the new dataset.
        features_to_drop = ['is_fraud', 'user_id', 'timestamp', 'timestamp_dt']
...
def tune_and_train_classifier_task():
    ...
    with mlflow.start_run(run_name=f"{DOMAIN_NAME}_classifier_tuning_and_training") as run:
        ...
        # Updated parameter grid aligned with the successful mobile_money DAG
        param_grid = {
            "n_estimators": [100, 200],
            "max_depth": [3, 5],
            "learning_rate": [0.05, 0.1],
            "scale_pos_weight": [10, 20]  # Increased to better handle imbalance
        }
        ...
        plt.barh(np.array(feature_names)[top_indices], importances[top_indices])
        plt.xlabel("XGBoost Feature Importance")
        plt.title(f"Top 15 Important Features ({DOMAIN_NAME.title()})")
        plt.tight_layout()
...
with DAG(
    ...
    doc_md=f"""
    ### {DOMAIN_NAME.title()} Fraud Classification Pipeline
    This pipeline uses enhanced behavioral features and hyperparameter tuning to train a robust XGBoost model for detecting bank fraud.
    """
) as dag:
